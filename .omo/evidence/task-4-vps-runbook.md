...
> 대상: VPS (develop 브랜치, docker-compose 운영)
...
```bash
# 초기 부트 로그 확인
docker-compose logs --tail=100 systemtrading

# 실시간 로그 팔로우
docker-compose logs -f systemtrading
```
...
```bash
docker-compose logs --tail=200 systemtrading | grep -E "Value|PBR|리밸런싱|매수|매도"
```
