...
- `RSI_SELL_THRESHOLD = 70`: RSI 과매수 기준 (walk-forward 검증: 70이 최적).
- `PROFIT_TARGET_PERCENT = 0.0`: 더 이상 매도 조건에 사용되지 않음 (walk-forward 검증: 수익 목표 0%가 최적).
- `TIME_STOP_LOSS_DAYS = 180`: 시간 손절 기준 (walk-forward 검증: 180일이 최적, MDD -35.39%).
- `USE_MA20_FILTER = False`: MA20>MA60 추세 필터 (walk-forward v3: OFF가 최적, +77.11% / MDD -22.85%).
- **매수 조건:** `close > MA200 AND RSI(2) < 3 AND 2일간 -5% 이상 하락` (MA20>MA60 조건 제거)
- **매도 조건:** `RSI > 70 AND close > breakeven_price` (수익 목표 제거, breakeven 이상 즉시 매도)
- **시간 손절:** 매수 후 180일 초과 시 강제 매도 (기본 활성화)
...
- **손절 비활성화:** `enable_stop_loss=False` 기본값 유지. 시간 손절(180일)이 가격 손절보다 유리함.
